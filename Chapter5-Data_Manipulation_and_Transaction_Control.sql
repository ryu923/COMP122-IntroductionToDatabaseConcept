...
-- 1. Preparation: Drop all previous tables
ALTER TABLE Orders
...
-- 2. Preparation: Setting new table and data for chapter5
...
-- 3. P144: The INSERT command for Nick Taylor
INSERT INTO Acctmanager
	VALUES('T500','NICK', 'TAYLOR', '05-SEP-09', 42000, 3500, 'NE');

-- 4. P146: The INSERT INTO command for Mandy Lopez
INSERT INTO Acctmanager
	VALUES('L500','MANDY', 'LOPEZ', '01-OCT-09', 47000, 1500, NULL);

-- 5. P148: Use an INSERT statement that applies a DEFAULT column option
INSERT INTO Acctmanager (amid, amfirst, amlast, amsal, amcomm, region)
	VALUES('J500','Sammie', 'Jones', 39500, 2000, 'NW');

-- 6. P149: Using the ON NULL clause with a column DEFAULT option
ALTER TABLE Acctmanager
	ALTER COLUMN amsal NUMERIC(8,2) NULL;

-- 7. P150: Add a virtual column
ALTER TABLE Acctmanager
	ADD amearn AS (amsal + amcomm);

-- 8. P151: Error caused by using a virtual column in an INSERT statement - ERROR! Because the [amearn] column is sum of two other columns (VIRTUL COLUMN is not arrowed)
--INSERT INTO Acctmanager (amid, amfirst, amlast, amsal, amcomm, region, amearn)
--	VALUES('D500','SCOTT', 'DAVIS', 53000, 6000, 'SE', 59000);

-- 9. P152: Solution for #8 - Eliminate last [amearn] data
INSERT INTO Acctmanager (amid, amfirst, amlast, amsal, amcomm, region)
	VALUES('D500','SCOTT', 'DAVIS', 53000, 6000, 'SE');

-- 10. P152: Error with a single quote in the value - ERROR! Because of the ['] single quote
--INSERT INTO Acctmanager (amid, amfirst, amlast, amsal, amcomm, region)
--	VALUES('M500','PEG', 'O'hara', 46000, 2000, 'SW');

-- 11. P153: Solution for #10 - Use [''] complete quete
INSERT INTO Acctmanager (amid, amfirst, amlast, amsal, amcomm, region)
	VALUES('M500','PEG', 'O''hara', 46000, 2000, 'SW');

-- 12. P155: INSERT INTO command with a subquery
INSERT INTO Acctbonus (amid, amsal, region)
	SELECT amid, amsal, region
	FROM Acctmanager;

-- 13. P157: Command to correct the Amedate column value
UPDATE Acctmanager
	SET amedate = '01-AUG-09'
	WHERE amid = 'J500';

-- 14. P157: UPDATE command to reassign regions
UPDATE Acctmanager
	SET region = 'W'
	WHERE region IN ('NE', 'NW');

-- 15. P158: Updating multiple columns
UPDATE Acctmanager
	SET amedate = '10-OCT-09',
		region = 'S'
	WHERE amid = 'L500';

-- 16. P159: Clear the Region column
-- Before UPDATE table, CONSTRAINT should be dropped.
ALTER TABLE Customers
	DROP CONSTRAINT customers_region_ck;

UPDATE Customers
	SET Region = '';

-- 17. P162: DELETE command to remove a row from the ACCTMANAGER table
